    ...
    for (int64_t ci = 0; ci < array->extnitems / array->chunknitems; ci++) {
        if (!array->filled) {
            ...
            if (blosc2_schunk_append_buffer(array->sc, rchunk,
                                            (size_t) array->extchunknitems * typesize) < 0) {
                return CATERVA_ERR_BLOSC_FAILED;
            }
            array->empty = false;
            array->nchunks++;
...
int caterva_blosc_array_to_buffer(caterva_ctx_t *ctx, caterva_array_t *array, void *buffer) {
    ...
    CATERVA_ERROR(caterva_blosc_array_get_slice_buffer(ctx, array, start, stop, array->shape, bbuffer));
    return CATERVA_SUCCEED;
}
...
int caterva_blosc_array_empty(caterva_ctx_t *ctx, caterva_params_t *params,
    ...
    ndcell_params *n_params = malloc(sizeof(ndcell_params));
    size_t size_p = sizeof(ndcell_params);
    n_params->array = *array;
    for (int i = 0; i < CATERVA_MAX_DIM; ++i) {
 //       ndcell_params->cellshape[i] = storage->properties.blosc.cellshape[i];
        memcpy(&n_params->cellshape[i], &storage->properties.blosc.cellshape[i], sizeof(int32_t));
    }
    blosc2_udfilter caterva_udfilter = {0};
    caterva_udfilter.id = 128;
    caterva_udfilter.forward = ndcell_encoder;
    caterva_udfilter.backward = ndcell_decoder;
    caterva_udfilter.params = n_params;

    memcpy(&cparams.udfilters[0], &caterva_udfilter, sizeof(blosc2_udfilter));
    for (int i = 1; i < BLOSC2_MAX_UDFILTERS; ++i) {
        memcpy(&cparams.udfilters[i], &ctx->cfg->udfilters[i - 1], sizeof(blosc2_udfilter));
    }
    ...
    memcpy(&dparams.udfilters[0], &caterva_udfilter, sizeof(blosc2_udfilter));
    for (int i = 1; i < BLOSC2_MAX_UDFILTERS; ++i) {
        memcpy(&dparams.udfilters[i], &ctx->cfg->udfilters[i - 1], sizeof(blosc2_udfilter));
    }
