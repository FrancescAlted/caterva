...
static int test_ndlz(void *data, int nbytes, int typesize, int ndim, caterva_params_t params, caterva_storage_t storage) {
    ...
    cfg.compcodec = BLOSC_UDCODEC_FORMAT;
    cfg.compcodecmeta = 128;
    ndlz_params *c_params = malloc(sizeof(ndlz_params));
    c_params->blockshape = storage.properties.blosc.blockshape;
    c_params->ndim = ndim;
    blosc2_codec ndlz8;
    ndlz8.compcode = 243;
    ndlz8.compver = 1;
    ndlz8.complib = 1;
    ndlz8.encoder = ndlz8_compress;
    ndlz8.decoder = ndlz8_decompress;
    ndlz8.compname = "ndlz8";
    blosc2_register_codec(&ndlz8);
    cfg.udcodecs[0] = ndlz8;
    ...
    cparams.compcode = BLOSC_UDCODEC_FORMAT;
    cparams.compcode_meta = 243;
...
/*
        ...
        /* Decompress  */
        dsize = blosc2_decompress_ctx(array->sc->dctx, data_out, chunksize + BLOSC_MAX_OVERHEAD, data_dest, dsize);
        if (dsize <= 0) {
